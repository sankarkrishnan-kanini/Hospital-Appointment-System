...
import { getClientAccountApi } from '@/lib/api/patient.api';
...
export default function PatientTopBar() {
  ...
  return (
    <div className="fixed top-0 left-0 right-0 z-50 h-12 bg-gradient-to-r from-[#4a1d96] to-[#7c3aed] flex items-center justify-between px-6 shadow-md">
      {/* Left — logo */}
      <div className="flex items-center gap-2.5">
        ...
        <span className="text-purple-200 text-xs ml-1 hidden sm:block">· Patient Portal</span>
      </div>

      {/* Right */}
      <div className="flex items-center gap-4">
        <p className="text-purple-200 text-xs hidden md:block">{dateStr}</p>

        <div className="w-px h-4 bg-white/20" />

        <div className="flex items-center gap-1.5">
          <UserRound size={13} className="text-purple-200" />
          <span className="text-white text-xs font-medium">
            {profile ? `${profile.firstName} ${profile.lastName}` : user?.email}
          </span>
          <span className="text-xs bg-white/20 text-white px-2 py-0.5 rounded-full font-medium ml-1">Patient</span>
        </div>
          ...
          {open && (
            <div className="absolute right-0 top-full mt-2 w-80 bg-white rounded-2xl shadow-xl border border-gray-100 z-50 overflow-hidden">
              <div className="flex items-center justify-between px-4 py-3 border-b border-gray-100">
                ...
                {unreadCount > 0 && (
                  <button onClick={markAllAsRead} className="text-xs text-[#7c3aed] hover:underline font-medium">
                    Mark all as read
                  </button>
                )}
              </div>
                ...
                {notifications.length === 0 ? (
                  ...
                  notifications.map((n) => (
                    <li key={n.id} onClick={() => !n.isRead && markAsRead(n.id)}
                      className={`px-4 py-3 flex gap-3 cursor-pointer hover:bg-gray-50 transition ${!n.isRead ? 'bg-[#f5f0ff]' : ''}`}>
                      <span className={`mt-1.5 w-2 h-2 rounded-full flex-shrink-0 ${!n.isRead ? 'bg-[#7c3aed]' : 'bg-gray-200'}`} />
